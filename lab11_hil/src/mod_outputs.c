...
void module_outputs_update(void)
{
    ...
    if (outputsState == CLOSED) {
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_10, GPIO_PIN_SET);
    }
    else if (outputsState == OPEN) {
...
void module_outputs_init(void)
{
    if (!_is_init)
    {
        ...
        static GPIO_InitTypeDef GPIO_Init_PC10;
        GPIO_Init_PC10.Pin = GPIO_PIN_10;
        //GPIO_Init_PC10.Pull = GPIO_PULLUP;
        GPIO_Init_PC10.Mode = GPIO_MODE_OUTPUT_PP;
        HAL_GPIO_Init(GPIOC, &GPIO_Init_PC10);
        // Initialise thread
